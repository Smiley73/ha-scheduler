...
async def async_reload_entry(hass: HomeAssistant, entry: SchedulerConfigEntry) -> None:
    """Reload config entry."""
...
async def async_remove_config_entry_device(
    hass: HomeAssistant,
    config_entry: SchedulerConfigEntry,
    ...
    for schedule_id in list(schedules.keys()):
        if (DOMAIN, schedule_id) in device_entry.identifiers:
            ...
            hass.config_entries.async_update_entry(
                config_entry, data={**config_entry.data, "schedules": schedules}
            )
