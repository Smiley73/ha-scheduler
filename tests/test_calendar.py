...
async def test_calendar_events_date_schedule(
    hass: HomeAssistant, mock_entry_with_schedules
):
...
async def test_calendar_events_week_schedule(
    hass: HomeAssistant, mock_entry_with_schedules
):
...
async def test_calendar_week_schedule_dow_boundaries(hass: HomeAssistant):
    ...
    assert (
        event.start.weekday() == 0
    ), f"Should start on Monday, got {event.start.strftime('%A')}"

    # Event should end on a Friday (weekday 4)
    assert (
        event.end.weekday() == 4
    ), f"Should end on Friday, got {event.end.strftime('%A')}"
...
async def test_calendar_week_schedule_weekend_boundaries(hass: HomeAssistant):
    ...
    assert (
        event.start.weekday() == 5
    ), f"Should start on Saturday, got {event.start.strftime('%A')}"

    # Event should end on a Sunday (weekday 6)
    assert (
        event.end.weekday() == 6
    ), f"Should end on Sunday, got {event.end.strftime('%A')}"
